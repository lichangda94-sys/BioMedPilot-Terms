...
SCHEMA_PATH = ROOT / "schemas" / "term_entry.schema.json"
EXAMPLES_DIR = ROOT / "examples" / "terms"
...
def test_term_examples_validate_against_schema(path: Path) -> None:
    data = load_json(path)
    VALIDATOR.validate(data)


@pytest.mark.parametrize("path", EXAMPLE_FILES, ids=lambda path: path.name)
def test_term_example_status_is_public_safe(path: Path) -> None:
    data = load_json(path)
...
@pytest.mark.parametrize("path", EXAMPLE_FILES, ids=lambda path: path.name)
def test_term_examples_do_not_use_forbidden_keys(path: Path) -> None:
    data = load_json(path)
